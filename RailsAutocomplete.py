...
class AutocompleteSettings:
	...
	def __init__(self, window):
		...
		self.project_folder_exists = self.project_folder != None
		self.enabled = False
	...
	def ensure_project_settings_has_autocomplete_settings(self):
		...
		if self.settings_key() in settings:
			...
			full_settings = dict( list(self.default_settings().items()) + list(settings[self.settings_key()].items()) )

			# save back to project settings if there are changes
			if settings[self.settings_key()] != full_settings:
			...
			settings[self.settings_key()] = self.default_settings()
			self.window.set_project_data(settings)
		...
		return settings[self.settings_key()]
...
class AutocompleteGeneratorThread(threading.Thread):
...
# end AutocompleteGeneratorThread
...
class AutocompleteGenerator(sublime_plugin.EventListener):
	...
	def on_post_save(self, view):
		...
		self.generator_thread = AutocompleteGeneratorThread(settings)
